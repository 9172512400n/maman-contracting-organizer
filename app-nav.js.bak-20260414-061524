...
<div style="font-size:11px;color:#888;">${[r.phone, r.address].filter(Boolean).join(' · ') || 'No details'}</div>
  ...
  function openJobModal(prefillDate) {
    ...
    modal.scrollTop = 0;
    // Hide bottom half initially — reduces paint complexity on open
    const bottom = document.getElementById('job-form-bottom');
    if (bottom) bottom.style.visibility = 'hidden';
    // Frame 1: paint modal backdrop
    requestAnimationFrame(function() {
      // Frame 2: clear + focus top fields immediately
      requestAnimationFrame(function() {
        ...
        // Show bottom after short delay — top fields are ready immediately
        setTimeout(function() {
